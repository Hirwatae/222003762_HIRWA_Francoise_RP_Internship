...
const LearnerDashboard = () => {
    ...
    useEffect(() => {
        ...
        if (storedUserId) {
            setCurrentUserId(parseInt(storedUserId, 10)); // Parse to int, specifying radix
        } else {
    ...
    // Effect to check enrollment status when a course is selected or currentUserId changes
    useEffect(() => {
        const checkEnrollmentStatus = async () => {
            if (currentUserId && selectedCourseId) {
                try {
                    const response = await axios.get(`${backendUrl}/enrollments/status`, {
                        params: {
                ...
                setIsEnrolled(false); // Reset if no course or user ID
            }
        };
    ...
    }, [currentCourse, currentNotePage, totalNotePages, learnerName, assessmentReady]);
    ...
    }, [viewingNotes, currentCourse, currentNotePage, backendUrl]); // Added backendUrl to dependencies
    ...
    const handleCourseSelect = (e) => {
        ...
        setError('');
        setMessage('');
        setIsEnrolled(false); // Reset enrollment status on course change, will be re-checked by useEffect
    };
    ...
    const handleEnroll = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${backendUrl}/enrollments`, {
                ...
                moduleId: parseInt(selectedCourseId, 10) // Use selectedCourseId
            });
    ...
    const handleStartCourse = () => {
        ...
        // The useEffect for notes will handle fetching the content for page 1
        setError('');
    ...
    const handlePrepareAssessment = async () => {
        ...
        try {
            ...
            const response = await axios.get(`${backendUrl}/assessments/${encodeURIComponent(courseTitleForAssessment)}`); // Encode URL component
            ...
            const errorMessage = err.response && err.response.data && err.response.data.error
                                     ...
                                     : 'Failed to load assessment questions. Please check your backend server and ensure questions are configured for this course.';
    ...
    const handleSubmitAssessment = async () => { // Made async to send results to backend
        ...
        const percentage = (totalPossibleMarks > 0) ? (earnedMarks / totalPossibleMarks) * 100 : 0; // Avoid division by zero
    ...
    const sendCompletionStatus = async (passed, score) => {
        if (!currentCourse || !selectedCourseId || !currentUserId) {
            ...
            // We already show an error to the user via setMessage, so just log here.
            return;
        }
    ...
    const fetchCourseNotesPage = async (courseTitle, pageNumber) => {
        ...
        setMessage(''); // Clear messages
        try {
            const response = await axios.get(`${backendUrl}/courses/${encodeURIComponent(courseTitle)}/notes/${pageNumber}`);
            setNoteContent(response.data.content);
            setTotalNotePages(response.data.totalPages || currentCourse.pages); // Use backend's totalPages if available, else currentCourse's
    ...
    const handleNextPage = () => {
        if (currentCourse && currentNotePage < totalNotePages) { // Use totalNotePages for boundary check
            ...
            <div className="dashboard-container">
                ...
                <div className="course-section">
                    ...
                    {isEnrolled && currentCourse && !viewingNotes && !assessmentActive && (
                        ...
                        >
                            {courseProgress > 0 && courseProgress < 100 ? 'Continue Reading Notes' : 'Start Course Notes'}
                        </button>
                    )}
                        ...
                        {loading ? (
                            ...
                            <>
                                ...
                                <div className="pagination-controls">
                                    ...
                                    <span> Page {currentNotePage} of {totalNotePages} </span> {/* Changed to totalNotePages */}
